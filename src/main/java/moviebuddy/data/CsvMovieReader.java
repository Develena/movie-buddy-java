...
public class CsvMovieReader extends AbstractMetadataResourceMovieReader implements MovieReader {
    ...
    public List<Movie> loadMovies() {
        ...
        Cache cache = cacheManager.getCache(getClass().getName()); // 캐시 객체 이름 : 현재 클래스 이름
...
//                    .collect(Collectors.toList());
            movies = new BufferedReader(new InputStreamReader(content, StandardCharsets.UTF_8)) // BufferedReader, InputStreamReader를 통해 읽기
                    .lines() // line by line으로 읽어들임
                    .skip(1)
                    .map(mapCsv)
                    .collect(Collectors.toList());
